...
Antigate.prototype.processFromFileWithComment = function (captchaFile, comment, callback) {
    var self = this;

    this.readCaptcha(captchaFile, function (error, data) {
        if (error) {
            callback(error, null);
        } else {
            return self.processWithComment(data, comment, callback);
        }
    });
...
Antigate.prototype.uploadWithComment = function (body, comment, callback) {
    request.post('http://antigate.com/in.php', {
        form: {
            method: 'base64',
            key: this.key,
